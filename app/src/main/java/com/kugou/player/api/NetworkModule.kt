...
object NetworkModule {
    ...
    private const val PREFS_NAME = "kugou_cookies"
    private const val KEY_TOKEN = "token"
    private const val KEY_USERID = "userid"
    private const val KEY_GUID = "device_guid"
    private const val KEY_MID = "device_mid"

    @Provides
    @Singleton
    fun provideCookieManager(@ApplicationContext context: Context): CookieManager {
        return CookieManager(context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE))
    }
    ...
    fun provideOkHttpClient(cookieManager: CookieManager): OkHttpClient {
        ...
        val cookieInterceptor = Interceptor { chain ->
            ...
            // Inject device cookies
            val guid = cookieManager.getOrCreateGuid()
            val mid = cookieManager.getOrCreateMid()
            val dev = "android"

            requestBuilder.addHeader("Cookie", "KUGOU_API_GUID=$guid; KUGOU_API_MID=$mid; KUGOU_API_DEV=$dev")

            // Forward auth cookies if logged in
